	...
	public static Result summarize(Result[] results, int ntheater) {
		...
		if (ntheater != 0) {
			for (int i = 0; i < ntheater; i++) {
				long theatertime = 0;
				for (int j = i; j < results.length; j += ntheater) theatertime += results[j].time;
				if (theatertime > twork) twork = theatertime;
			}
		} else {
			for (int i = 0; i < results.length; i++) if (results[i].time > twork) twork = results[i].time;
		}
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (dx != 0.0 || dy != 0.0 || dz != 0.0) {
				...
				if (dis < r.min) {
					...
					for (int j = 0; j < 3; j++) r.minstar[0][j] = star[j];
				} else if (dis == r.min) {
					double[][] t = new double[r.minstar.length+1][3];
					for (int j = 0; j < r.minstar.length; j++) for (int k = 0; k < 3; k++) t[j][k] = r.minstar[j][k];
				...
				if (dis > r.max) {
					...
					for (int j = 0; j < 3; j++) r.maxstar[0][j] = star[j];
				} else if (dis == r.max) {
					double[][] t = new double[r.maxstar.length+1][3];
					for (int j = 0; j < r.maxstar.length; j++) for (int k = 0; k < 3; k++) t[j][k] = r.maxstar[j][k];
