...
<body>
    ...
    <%
    	...
    	System.out.println(new Date().toString() + " begin data collection for query " + query);
...
//    	System.out.println("cat sim calculated for query " + query);
...
//    	ArrayList<StringDoublePair> simRelationPropW = RelationLinkFinder.getPropSimW(queryID);	    		    				
		ArrayList simRelationPropC = RelationLinkFinder.getPropSimC(queryID);
    	ArrayList simRelationPropW = RelationLinkFinder.getPropSimW(queryID);	    		    				
	    
//    	System.out.println("prop sim calculated for query " + query);

    	//Similar Relation Via Ins
//    	ArrayList<StringDoublePair> simRelationInsC = RelationLinkFinder.getInsSim(queryID);
    	ArrayList simRelationInsC = RelationLinkFinder.getInsSim(queryID);
    	
//    	System.out.println("ins sim calculated for query " + query);
...
//    	ArrayList<SubObjNum> instances = RelationInstanceFinder.findInstance(queryID, 0, 20);
    	ArrayList relationInfo = RelationInfoRetriever.getCategoryPair(level, queryID);
    	ArrayList instances = RelationInstanceFinder.findInstance(queryID, 0, 20);
		
//    	System.out.println("misc data calculated for query " + query);
...
//		ArrayList<StringDoublePair> simRelationPropExW = new ArrayList<StringDoublePair>();	    	
		ArrayList simRelationPropExC = new ArrayList();
		ArrayList simRelationPropExW = new ArrayList();	    	
	   	RelationLinkFinder.getPropExSim(query, simRelationPropExC, simRelationPropExW);
	    	
//    	System.out.println("propEx sim calculated for query " + query);
    	
    	System.out.println(new Date().toString() + " data collected for query " + query);
    %>
...
<table width="913" border="2" align = "center" bordercolor="#6666FF">
	...
	<tr>
        ...
        int count = 0;
		for (Iterator i = simRelationCatC.iterator(); i.hasNext(); ) {
			StringDoublePair sdp = (StringDoublePair)i.next();
		...
		count = 0;
		for (Iterator i = simRelationCatW.iterator(); i.hasNext(); ) {
			StringDoublePair sdp = (StringDoublePair)i.next();
			out.println("<a href = \"./search.jsp?query=" + sdp.s + "&level=" + level
					+ "\">" + sdp.s + "</a>\t\t" + sdp.d + "<br />" );
			count++;
		...
		count = 0;
		for (Iterator i = simRelationPropC.iterator(); i.hasNext(); ) {
			StringDoublePair sdp = (StringDoublePair)i.next();
			out.println("<a href = \"./search.jsp?query=" + sdp.s + "&level=" + level
					+ "\">" + sdp.s + "</a>\t\t" + sdp.d + "<br />" );
			count++;
		...
		count = 0;
		for (Iterator i = simRelationPropW.iterator(); i.hasNext(); ) {
			StringDoublePair sdp = (StringDoublePair)i.next();
			out.println("<a href = \"./search.jsp?query=" + sdp.s + "&level=" + level
					+ "\">" + sdp.s + "</a>\t\t" + sdp.d + "<br />" );
			count++;
		...
		count = 0;
		for (Iterator i = simRelationPropExC.iterator(); i.hasNext(); ) {
			StringDoublePair sdp = (StringDoublePair)i.next();
			out.println("<a href = \"./search.jsp?query=" + sdp.s + "&level=" + level
					+ "\">" + sdp.s + "</a>\t\t" + sdp.d + "<br />" );
			count++;
		...
		count = 0;
		for (Iterator i = simRelationPropExW.iterator(); i.hasNext(); ) {
			StringDoublePair sdp = (StringDoublePair)i.next();
			out.println("<a href = \"./search.jsp?query=" + sdp.s + "&level=" + level
					+ "\">" + sdp.s + "</a>\t\t" + sdp.d + "<br />" );
			count++;
		...
		count = 0;
		for (Iterator i = simRelationInsC.iterator(); i.hasNext(); ) {
			StringDoublePair sdp = (StringDoublePair)i.next();
			out.println("<a href = \"./search.jsp?query=" + sdp.s + "&level=" + level
					+ "\">" + sdp.s + "</a>\t\t" + sdp.d + "<br />" );
			count++;
		...
		<td valign="top">
			<div align="left">
				...
				int k = 0;
				for (Iterator i = relationInfo.iterator(); i.hasNext(); ) {
					SubObjNum son = (SubObjNum)i.next();
		...
		<td valign="top">
			<div align="left">
				<%
				out.println("<strong>Subject\t|\tObject</strong> <p>");
				k = 0;
				for (Iterator i = instances.iterator(); i.hasNext(); ) {
					SubObjNum son = (SubObjNum)i.next();
					out.println(URLDecoder.decode(Translator.getResName(son.sub), "utf-8").replaceAll("_", " ") + "\t|\t" 
						+ URLDecoder.decode(Translator.getResName(son.obj), "utf-8").replaceAll("_", " ") + " <p />");
					k++;
					if (k >= 20) break;
				}
				%>
