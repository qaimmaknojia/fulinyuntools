...
public class BallFinder {
	...
	public static void find(int n, int k) {
		...
		for (int i = 1; i <= n / 4 + 1; i++) {
			root.onbalance = new int[2 * i];
			for (int j = 0; j < 2 * i; j++)
				root.onbalance[j] = j;

			experiment(root.onbalance, p, -1, k);
			root.left = new Exp();
			if (!find(n, k - 1, p, i-1, root.left)) {
				Arrays.fill(p, BAD);
				notdone = true;
				continue;
			} else
			...
			experiment(root.onbalance, p, 0, k);
			root.even = new Exp();
			if (!find(n, k - 1, p, i-1, root.even)) {
				Arrays.fill(p, BAD);
				notdone = true;
				continue;
			} else
			...
			experiment(root.onbalance, p, 1, k);
			root.right = new Exp();
			if (!find(n, k - 1, p, i-1, root.right)) {
				Arrays.fill(p, BAD);
				notdone = true;
				continue;
			} else
	...
	private static void printTree(Exp node, int layer) {
		...
		else if (node.onbalance.length == 1)
			System.out.println(node.onbalance[0]);
		else {
		...
		for (int i = 1; i <= max; i++) {
			...
			while (true) {
				...
				if (experiment(current.onbalance, p, -1, k)) {
					...
					if (!find(n, k - 1, p, i-1, current.left)) {
						for (int l = 0; l < p.length; l++)
							p[l] = temp[l];
				...
				if (experiment(current.onbalance, p, 0, k)) {
					...
					if (!find(n, k - 1, p, i-1, current.even)) {
						for (int l = 0; l < p.length; l++)
							p[l] = temp[l];
						if (!add1(current.onbalance, n))
							break;
				...
				if (experiment(current.onbalance, p, 1, k)) {
					...
					if (!find(n, k - 1, p, i-1, current.right)) {
						for (int l = 0; l < p.length; l++)
							p[l] = temp[l];
						if (!add1(current.onbalance, n))
							break;
	 ...
	 */
	private static boolean experiment(int[] onbalance, int[] p, int result,
			int layer) {
